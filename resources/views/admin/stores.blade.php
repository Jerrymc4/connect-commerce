...
<div class="bg-white rounded-lg shadow-sm p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.stores.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition flex items-center">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="relative max-w-xs w-full">
            <input type="text" name="search" class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" placeholder="Search stores...">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 text-center">
            ...
            <p class="text-xl font-bold text-gray-900 mt-1">{{ $totalStores ?? 0 }}</p>
        </div>
        <div class="bg-green-50 rounded-lg p-4 border border-green-200 text-center">
            <p class="text-sm font-medium text-green-600">Active Stores</p>
            <p class="text-xl font-bold text-green-800 mt-1">{{ $activeStores ?? 0 }}</p>
        </div>
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10 rounded-md bg-gray-200 flex items-center justify-center">
                                    @if($store->logo)
                                        <img src="{{ $store->logo }}" alt="{{ $store->name }}" class="h-10 w-10 rounded-md object-cover">
                                    @else
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-sm text-gray-500">Created: {{ $store->created_at->format('M d, Y') }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $store->domain }}</div>
                            <a href="https://{{ $store->domain }}" target="_blank" class="text-xs text-blue-600 hover:text-blue-800">
                                <i class="fas fa-external-link-alt mr-1"></i> Visit
                            </a>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $store->owner_name }}</div>
                            <div class="text-sm text-gray-500">{{ $store->owner_email }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $store->products_count ?? 0 }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $store->orders_count ?? 0 }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <a href="{{ route('admin.stores.view', $store->id) }}" class="text-blue-600 hover:text-blue-900">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.stores.edit', $store->id) }}" class="text-gray-600 hover:text-gray-900">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form method="POST" action="{{ route('admin.stores.delete', $store->id) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this store? This action cannot be undone and will delete all associated data.')">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Showing <span class="font-medium">1</span> to <span class="font-medium">10</span> of <span class="font-medium">{{ isset($storesCount) ? $storesCount : '0' }}</span> stores
                    </p>
                </div>
                <div>
